...
async function averageRating(req, res, next) {
  // your solution here
  const ratingAverage =  await service.averageRating();
  ratingAverage["average_rating"] = Number(ratingAverage.average_rating)
  res.json({ data: ratingAverage });
}
...
async function count(req, res, next) {
  // your solution here
  const theCount = await service.count();
  theCount["count"] = Number(theCount.count)
  res.json({ data: theCount });
}
